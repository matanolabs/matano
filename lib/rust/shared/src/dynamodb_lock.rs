...
//! Adapted from https://github.com/delta-io/delta-rs/blob/main/dynamodb_lock/src/lib.rs
//! removing rusoto dep!
...
pub enum DynamoError {
    ...
    DeleteItemError(#[from] DeleteItemError),
...
impl From<SdkError<PutItemError>> for DynamoError {
    fn from(error: SdkError<PutItemError>) -> Self {
        let se = error.into_service_error();
        match se {
            PutItemError::ConditionalCheckFailedException(_) => DynamoError::ConditionalCheckFailed,
            PutItemError::ProvisionedThroughputExceededException(_) => {
                DynamoError::ProvisionedThroughputExceeded
            }
            _ => DynamoError::PutItemError(se),
        }
    }
}

impl From<SdkError<GetItemError>> for DynamoError {
    fn from(error: SdkError<GetItemError>) -> Self {
        let se = error.into_service_error();
        println!("**** GetItemError: {:?}", se);
        match se {
            GetItemError::ResourceNotFoundException(_) => DynamoError::TableNotFound,
            GetItemError::ProvisionedThroughputExceededException(_) => {
                DynamoError::ProvisionedThroughputExceeded
            }
            _ => DynamoError::GetItemError(se),
        }
    }
}

/// The partition key field name in DynamoDB
pub const PARTITION_KEY_NAME: &str = "pk";
...
impl DynamoDbLockClient {
    ...
    pub async fn get_lock(&self) -> Result<Option<LockItem>, DynamoError> {
        let output = self
            ...
            .key(
                PARTITION_KEY_NAME,
                attr(self.opts.partition_key_value.clone()),
            )
            .send()
            .await?;
        ...
        if let Some(item) = output.item {
            let lease_duration = {
                match item.get(LEASE_DURATION).and_then(|v| v.as_s().ok().clone()) {
                    None => None,
                    Some(v) => Some(
                        v.parse::<u64>()
            ...
            let data = item
                .get(DATA)
                .and_then(|r| r.as_s().ok().map(|s| s.to_owned()));
    ...
    async fn delete_item(&self, rvn: &str, owner: &str) -> Result<bool, DynamoError> {
        let result = self
            ...
            .set_expression_attribute_names(Some(hashmap! {
                vars::PK_PATH.to_string() => PARTITION_KEY_NAME.to_string(),
                vars::RVN_PATH.to_string() => RECORD_VERSION_NUMBER.to_string(),
                vars::OWNER_NAME_PATH.to_string() => OWNER_NAME.to_string(),
            }))
        ...
        match result.await {
            ...
            Err(e) => {
                ...
                if se.is_conditional_check_failed_exception() {
                    ...
                    Err(DynamoError::DeleteItemError(se))
                }
            }
        }
    }
