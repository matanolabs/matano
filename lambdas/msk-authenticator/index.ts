...
class Sha256HashConstructor implements Hash {
  sha256: Hmac;
  constructor(secret: SourceData | undefined) {
    if (secret === undefined) {
      return
    }
    this.sha256 = createHmac("sha256", secret?.toString());
  }
  ...
  update(toHash: BinaryLike, encoding: any) {
...
export const mskIamAuthenticator: (
  region: string
) => Mechanism["authenticationProvider"] =
  (region) => (host, port, logger, saslAuthenticate) => {
    const INT32_SIZE = 4;

    const request = (payload: Payload): SaslAuthenticationRequest => ({
      encode: () => {
    ...
    return {
      authenticate: async () => {
        ...
        try {
          ...
          const authenticateResponse = await (saslAuthenticate as any)({
