...
function prepareCdkPkg() {
  ...
  const cdkPkgConfig = {
	...
	targets: [
	  ...
	  "node14.18.1-linux-arm64",
	  "node14.18.1-macos-arm64"
	],
...
async function main() {
  ...
  for (const target of ["linux", "macos"]) {
	for (const arch of ["x64", "arm64"]) {
	  const targetSubDir = path.join(workOutputDir, `${target}-${arch}`);
	  execSync(`mkdir -p ${targetSubDir}`);

	  const executables = (await fs.promises.readdir(workOutputDir)).filter((fn) => fn.includes(`-${target}-${arch}`));
	  for (const executable of executables) {
		const simpleExecutableName = executable.replace(`-${target}-${arch}`, "");
	  ...
	  const archiveName = `matano-${target}-${arch}.sh`;
	  execSync(
